package com.boc.database.javase.FanXing;

public class Main2 {
    public static void main(String[] args) throws InstantiationException, IllegalAccessException {
        System.out.println("1-----------");
        Dog dog1 = creatNew1(Dog.class);//creatNew1(Class<T> clazz)
        System.out.println(dog1.name + " is " + dog1.type+ "\n");

        System.out.println("2-----------");
        Animal ani1 = creatNew2(Cat.class);//creatNew2(Class<? extends Animal> clazz)
        Cat cat1 = (Cat) ani1;
        System.out.println(cat1.name + " is " + cat1.type + "\n");

        System.out.println("3-----------");
        Object obj = creatNew3(Dog.class);//creatNew3(Class clazz)
        Dog dog2 = (Dog) obj;
        System.out.println(dog1.name + " is " + dog2.type + "\n");

        System.out.println("4-----------");
        Object obj2 = creatNew4(Cat.class);// creatNew4(Class<?> clazz)
        Cat cat2 = (Cat) obj2;
        System.out.println(cat2.name + " is " + cat2.type + "\n");

        System.out.println("5-----------");
        Object obj3 = creatNew5(new Cat());//creatNew5(Object object)
        Cat cat3 = (Cat) obj3;
        System.out.println(cat3.name + " is " + cat3.type + "\n");
    }

    /**
     * 此方法是一个非泛型类中的泛型方法，参数为Class<T>类型，可以传入任何类，但是Class<T>将参数在函数内部的类型固定为 T类，
     * 使用clazz.newInstance(),返回的类型也为固定的 T 类型。
